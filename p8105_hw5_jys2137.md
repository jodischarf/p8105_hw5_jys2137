...
To make sure missing has been replaced correctly, we are doing a check
on the data frame to see if there are any rows with NA values in the
dataframe.
...
The dataframe looks like it does not include any more NA values.
Finally, we can do a test of the new dataset.
...
Great, no NA values to be found!
