...
* **1.5.** Focusing on Baltimore, MD
* **1.6.** Running `prop.test` for each of the cities in the dataset
* **1.5.** Creating a plot: Proportion of unsolved cases in 50 major US cities
...
Next, we run `prop.test` for each of the cities in the dataset, and extract both the proportion of unsolved homicides and the confidence interval for each. This is done within a “tidy” pipeline, making use of `purrr::map`, `purrr::map2`, and `unnest` as necessary to create a tidy dataframe with **estimated proportions** and **CIs* for each city.
